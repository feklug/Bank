...
Integriert Schweizer Feiertagskalender (Bund + Kanton Zürich) für next_expected_date.
Ausgabe: Konsole (strukturiert)
...
def get_swiss_holidays_zh(year: int) -> set[date]:
    ...
    Bundesfeiertage (CH) + kantonale Feiertage ZH.
    """
    easter = _easter(year)
    holidays = {
        ...
        date(year, 8, 1),                        # Bundesfeiertag (Schweizer Nationalfeiertag)
...
def adjust_to_banking_day(d: date) -> date:
    """Falls Datum kein Bankarbeitstag -> naechster Bankarbeitstag."""
...
def next_month_same_day(last_date: date, anchor_dom: int) -> date:
    """
    Naechster Monat, am anchor_dom (day-of-month).
    Clip auf letzten Monatstag falls noetig (z.B. 31. -> Feb -> 28.).
    """
...
def next_quarter_same_dom(last_date: date, anchor_dom: int) -> date:
    """Naechstes Quartal, am anchor_dom (day-of-month). Clip auf Monatsende."""
    m = last_date.month + 3
...
def compute_next_expected(dates: list[date], interval_label: str | None,
    ...
    """
    Berechnet next_expected_date.
    Fuer MONTHLY: next_month_same_day() -> kein Datums-Drift.
    Fuer QUARTERLY: next_quarter_same_dom().
    Fuer alle anderen: last_date + interval_days.
    """
...
def load_transactions(path: str) -> list[dict]:
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    for t in data:
        t["date_obj"] = date.fromisoformat(t["datum"])
...
def group_key(t: dict) -> tuple:
    ...
    """
    iban = t.get("iban") or ""       # [B1] None-safe: None und "" beide -> ""
...
def detect_interval(dates: list[date]) -> tuple[str | None, int | None, float]:
    """
    Erkennt das dominante Intervall aus einer Liste von Datumswerten.
    Gibt (label, interval_days, date_confidence) zurueck.
    date_confidence = Anteil Gaps innerhalb der Toleranz.
    """
    if len(dates) < 2:
...
def is_seasonal(dates: list[date], interval_label: str | None) -> bool:
    """Prueft ob Zahlungen saisonal sind (immer in gleichen Monaten)."""
    if len(dates) < 2:
        return False
...
def detect_amount_anomalies(amounts: list[float], tolerance_factor: float = 2.0) -> list[int]:
    """Gibt Indizes zurueck, bei denen der Betrag stark abweicht (z-Score)."""
    if len(amounts) < 3:
        return []
    avg = mean(amounts)
    try:
...
def detect_date_anomalies(dates: list[date], interval_days: int) -> list[int]:
    """
    Gibt Indizes zurueck, wo das Datum stark vom erwarteten Intervall abweicht.
    Toleranz proportional: 10 % des Intervalls, mindestens 3 Tage.  [V1]
    """
...
def detect_iban_anomalies(transactions: list[dict]) -> list[int]:
    """
    Gibt Indizes zurueck, wo die IBAN vom Gruppen-Dominant abweicht.  [V5]
    """
...
def detect_duplicates(transactions: list[dict]) -> list[int]:
    """
    Gibt Indizes zurueck, die wahrscheinliche Duplikate sind:
    Gleiche Gegenpartei, gleicher Betrag (+-1 %), Abstand <= 10 Tage.  [V6]
    """
    duplicates = set()
...
def compute_amount_trend(amounts: list[float]) -> float | None:
    """
    Berechnet den linearen Trend (CHF pro Periode) ueber die Betragsreihe.  [V3]
    Positiver Wert = Betrag steigt (z.B. Gehaltserhöhung).
    Gibt None zurueck wenn zu wenige Datenpunkte.
    """
    if len(amounts) < 3:
        return None
    try:
...
def forecast_confidence(date_conf: float, amounts: list[float],
    ...
    """
    Forecast-Konfidenz: kombiniert Datums- UND Betrags-Regelmaessigkeit.
    Formel: 60 % Datum-Konfidenz + 40 % Betrags-Konfidenz
    IBAN-Bonus: +0.15 wenn alle IBAN identisch.
    Exakt-Betrag-Bonus: +0.10 wenn Betrag exakt gleich ueber >= 3 Perioden.
    """
    sd = 0.0  # Initialisierung vor dem if-Block, verhindert UnboundLocalError  [FIX]
...
def find_sequential_patterns(transactions: list[dict], used_ids: set[int]) -> list[dict]:
    """Sucht kausale A->B Sequenzen anhand vordefinierter Regeln."""
    patterns = []
...
# Mindest-Datum-Konfidenz fuer Muster-Erkennung je Intervalltyp  [V8]
MIN_DATE_CONF: dict[str, float] = {
    "CUSTOM":  0.70,
    "DEFAULT": 0.40,
}

# Mindest-Konfidenz fuer Aufnahme ins Pattern-Ergebnis  [V10]
# Beide Schwellen muessen erfuellt sein, sonst -> OHNE MUSTER
...
def analyze(path: str):
    ...
    for key, indexed_txs in groups.items():
        ...
        # Mindest-Datum-Konfidenz je Intervalltyp  [V8]
        ...
        # Konfidenz-Schwelle 75 %: beide Werte muessen >= 0.75 sein  [V10]
        # Andernfalls bleiben die Transaktionen unmarkiert -> OHNE MUSTER
    ...
    # C) OHNE MUSTER (inkl. verworfene Muster + interne Umbuchungen)
...
def print_anomalies(p: dict):
    ...
    for a in anomalies:
        zeit_str = f"  {a['zeit']}" if a.get("zeit") else ""
        print(f"     {a['date']}{zeit_str}  {fmt_amount(abs(a['amount']))}  [{a['type']}]  {a['description'][:40]}")
...
def print_no_pattern(txs: list[dict]):
    ...
    for t in txs:
        ...
        zeit_str = f"  {t['zeit']}" if t.get("zeit") else ""
        print(f"  {t['datum']}{zeit_str}  {sign}{t['betrag']:>10,.2f} CHF  {t['gegenpartei'][:30]:<30}  {t['category_level1']}")
