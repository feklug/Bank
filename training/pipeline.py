...
Datenschutz-Modus (Standard: AN):
  - Alle Konsolenausgaben der Subskripte werden unterdrückt
  - Zwischendateien (tink_categorized.json, tink_patterns.json) werden
    nach Abschluss automatisch gelöscht → keine Transaktionsdaten im Repo/Log
  - Nur Statusmeldungen der Pipeline selbst (Step X ✅, Dauer) bleiben sichtbar

  Deaktivieren: Env-Variable SILENT_MODE=false setzen

Konfiguration via Env-Variablen:
  INPUT_FILE                      (default: tink_demo_transactions.json)
  OUTPUT_DIR                      (default: . )
...
TEMP_FILES  = [CATEGORIZED, PATTERNS]   # werden nach Abschluss gelöscht
...
def _check_file(path: str, produced_by: str):
    """Prüft ob Datei existiert – gibt keine Pfade/Größen aus (Datenschutz)."""
...
def _cleanup():
    """Löscht alle Zwischendateien mit Transaktionsdaten."""
...
def _load(name: str):
    ...
    path = ROOT / f"{name}.py"
    if not path.exists():
        print(f"\n❌  Skript nicht gefunden: {path.name}")
        sys.exit(1)
...
def _run(fn, *args, **kwargs):
    """
    Führt fn() aus.
    Im SILENT_MODE werden stdout + stderr der Subskripte nach /dev/null umgeleitet.
    Fehler (Exceptions, sys.exit != 0) werden trotzdem weitergegeben.
    """
    if SILENT_MODE:
        sink = io.StringIO()
...
def run_calculate_unknown():
    ...
    try:
        ...
        # Nur Zählwerte ausgeben, keine Transaktionsdaten
        if result:
...
def main():
    ...
    print(f"\n{SEP}")
    print(f"  PIPELINE START  {'[SILENT]' if SILENT_MODE else '[VERBOSE]'}")
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(SEP)
    ...
    try:
        ...
        # Zwischendateien immer löschen – auch bei Fehler
        if SILENT_MODE:
            _cleanup()
